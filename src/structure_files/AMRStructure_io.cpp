...
int AMRStructure::write_particles_to_file(bool pre_remesh, int iter_num) {
    ...
    std::ofstream u1_file;
    std::ofstream u2_file;
    ...
    y_file.open(sim_dir + "simulation_output/" + species_name + "/ys/ys_" + remesh_str  + std::to_string(iter_num), std::ios::out | std::ios::binary); 
    ...
    u1_file.open(sim_dir + "simulation_output/" + species_name + "/u1s/u1s_" + remesh_str  + std::to_string(iter_num), std::ios::out | std::ios::binary); 
    u2_file.open(sim_dir + "simulation_output/" + species_name + "/u2s/u2s_" + remesh_str  + std::to_string(iter_num), std::ios::out | std::ios::binary); 
    ...
    if (!x_file || !y_file || !f_file || !qw_file || !u1_file || !u2_file) {
    ...
    for (int ii = 0; ii < xs.size(); ++ii) {
        ...
        y_file.write((char *) &y, sizeof(double));
        f_file.write((char *) &f, sizeof(double));
    }



    // assert(xs.size() == ys.size() && ys.size() == fs.size() && fs.size() == q_ws.size() && q_ws.size() == u1s.size() && u1s.size() == u2s.size() );
    // for (int ii = 0; ii < xs.size(); ++ii) {
    //     double x = xs[ii];
    //     double y = ys[ii];
    //     double f = fs[ii];
    ...
    //     u1_file.write((char *) &u1s, sizeof(double));
    //     u2_file.write((char *) &u2s, sizeof(double));
    // }

    if (!x_file.good() | !y_file.good() | !f_file.good() | !qw_file.good() | !u1_file.good() | !u2_file.good()) {
...
std::ostream& operator<<(std::ostream& os, const AMRStructure& amr) {
    ...
    // os << "Species charge: " << amr.y << ", species mass: " << amr.y/amr.qm << endl;
...
void AMRStructure::print_panel_points() {
    ...
    for (int ii = 0; ii < panels.size(); ii++) {
        ...
        cout << endl << "ps: ";
        for (int jj = 0; jj < 9; jj++) {
            cout << ys[panel_it->point_inds[jj]] << ", ";
        }
        cout << endl << "fs: ";
        for (int jj = 0; jj < 9; jj++) {
