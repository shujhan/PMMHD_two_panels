                                 ...
                                 bool do_adaptive_refine, bool is_initial_step) 
{
    ...
    if (is_initial_step) {
        ...
        interpolate_to_initial_xys(q0s,xs,ys, nx_points, ny_points);
        #ifdef DEBUG
...
void AMRStructure::recover_wj_from_q(const AMRStructure* q_minus_struct) {
    if (q0s.size() != q_minus_struct->q0s.size()) {
        std::cerr << "Error: q0s sizes do not match in recover_wj_from_q()" << std::endl;
        return;
    }
    ...
    for (size_t i = 0; i < q0s.size(); i++) {
        w0s[i] = 0.5 * (q0s[i] + q_minus_struct->q0s[i]);
        j0s[i] = 0.5 * (q0s[i] - q_minus_struct->q0s[i]);
    }
}


void AMRStructure::copy_wj_from(const AMRStructure* other) {
    w0s = other->w0s;   // vector copy 
    j0s = other->j0s;
}
...
void AMRStructure::update_panel_positions_and_q(const std::vector<double>& vx,
                                                const std::vector<double>& vy,
                                                const std::vector<double>& source,
                                                double dt,
                                                int sign) {
    if (xs.size() != vx.size() || ys.size() != vy.size() || q0s.size() != source.size()) {
